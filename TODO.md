...
- [ ] Add real low-text OCR fallback with Google Vision.
  ...
  - Partial: bibliography extraction now records symbol-heavy unreadable text pages with `ocr_recommended=true` when a PDF has a corrupt but non-empty text layer, and can use Google Vision visual OCR as a tail-page rescue when PDF span/page-text bibliography extraction finds nothing. Remaining work is real OCR execution for whole-page low-text and corrupt-text-layer page text, plus persistence of rescued OCR page text.
