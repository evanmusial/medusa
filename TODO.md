...
- [ ] Add tag Delete workflow.
  ...
  - Partial: Optimize now uses `gpt-5.4-mini` plus deterministic governance analysis to produce reviewable merge, relationship, status, and pruning suggestions. Approved merges and pruning run through audited document-history paths, and the plan pane can approve individual suggestions or batch-approve all current suggestions while reporting stale skipped actions.
