...
- [ ] Add tag Delete workflow.
  ...
  - Partial: Optimize now uses `gpt-5.4-mini` plus deterministic governance analysis to produce reviewable merge, orphaned-tag cleanup, relationship, status, and pruning suggestions. Approved merges and document-assignment pruning run through audited document-history paths; true zero-link orphan tags can be alias-merged into useful used tags or pruned entirely through a guarded approval path. The plan pane can approve individual suggestions or batch-approve all current suggestions while showing top progress feedback and reporting stale skipped actions.
