...
Settings includes full database backup and restore controls. Backups are named with `YYYYMMDD-HHMMSS` plus the short hostname, compressed with zstd, uploaded to GCS, and checksum-verified after upload. The Backup Database control shows a likely backup size based on current PostgreSQL database size and the latest completed backup when available, and Settings shows recent backup/restore run history instead of only the newest row. Restore from GCS or an uploaded dump is gated by a fresh verified safety backup, then applies with `pg_restore` and runs migrations. A full database restore may replace session rows, so the browser may need to sign in again afterward.
