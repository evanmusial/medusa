...
- Reserved header progress control for imports, Concordance, and citation-refresh work, so job feedback continues after navigating away from the page that started the work without shifting the header actions; active imports show count/percent, ETA, and known spend, and the latest completed import batch stays visible with actual cost until dismissed
