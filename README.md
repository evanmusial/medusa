...
The exporter serves `/metrics` and `/healthz` from `python -m app.tools.prometheus_exporter`, defaulting to port `43737`. On a server, set `MEDUSA_METRICS_BIND_IP` to the monitoring-only interface such as `23.227.185.85` for `medusa.evan.engineer`; inside the container, `MEDUSA_METRICS_BIND_HOST` should remain `0.0.0.0`. `/metrics` requires `Authorization: Bearer <token>` from `MEDUSA_METRICS_BEARER_TOKEN` or the preferred ignored token file `MEDUSA_METRICS_BEARER_TOKEN_FILE=/app/data/secrets/prometheus-token`. Set the same `MEDUSA_METRICS_INTERNAL_TOKEN` for the backend and exporter to enable the private `/api/internal/metrics/snapshot` feed for backend process/cache/release/maintenance metrics. The Docker socket mount in `docker-compose.metrics.yml` stays commented out unless container-level Docker Engine metrics are worth the host-control trust boundary. The exporter intentionally skips node-exporter-style host metrics and GCS bucket inventory/cost metrics.
