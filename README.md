...
- Host-agent-compatible release and maintenance status checks with authenticated `Upgrade Now`, `Reload Now`, `Check Now`, and `Run Maintenance Now` controls; maintenance applies require idle/active-work gates plus a verified full PostgreSQL backup before any Compose rebuild or runtime refresh
...
Dependency freshness is tracked through `renovate.json` and the twice-weekly operating plan in `docs/DEPENDENCY_UPDATE_PLAN.md`. Enable the Renovate GitHub app or a Renovate runner for the repo so the checked-in policy actually opens PRs. The host release agent can run an idle-gated Tuesday/Friday maintenance lane for already-merged safe updates and same-tag image/base rebuild refreshes, but it must complete and verify a fresh full GCS PostgreSQL backup before any Docker or PostgreSQL patching command runs. Runtime image updates, including Valkey, should keep explicit tags, preserve the private cache network, and verify that HAProxy remains the only host-published service on port `3737`. Docker Engine and Compose plugin updates are report-only host recommendations, not auto-applied by Medusa.
