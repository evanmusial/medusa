...
When the internal frontend service has no healthy HAProxy backend server during a restart, HAProxy serves a branded Medusa waiting page for normal browser routes and refreshes it automatically. `/api/*` requests still fail while the app is unavailable, and the waiting page carries `X-Medusa-Restarting: true` so frontend readiness checks cannot mistake it for the real app shell.
