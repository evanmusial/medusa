...
- Profile-menu Release History page at `/release-history`, backed by host-agent release records with date/time, commit hash, and concise feature-level change notes
...
- Library detail in-place document replacement, which uploads a new supported source file onto the same document record, preserves prior `DocumentVersion`, processing events, composition/cost rows, domains, project links, priority, and read state, clears source-derived content/tags/attributes and page-specific annotations, and queues a full replacement import without creating a duplicate Library item
- Portfolio assignment cockpit for user-authored research and school papers, with PDF/DOCX/RTF/TXT/Markdown draft/version uploads, labeled rubric/assignment/guide/reference/feedback/source materials, Library-resource suggestions, GPT-backed rubric scorecards and grade estimates, tamper-evident signed audit events with optional RFC 3161 timestamp anchors, generated previews, source downloads, and audited ZIP bundle export
...
- First-class publication metadata for journals, books, textbooks, proceedings, and report series, with publication identifiers, per-document appearance details, manual edit/verify/refresh controls, and Library browse-by-publication filtering
- Live document-level publication, DOI, citation, summary, bibliography, tag, and formula-capture controls backed by durable Concordance jobs, including Summary Validate for preserving exemplar summaries
...
- Document correction pane with configurable sticky identity fields, organized preview/Reader actions, metadata correction, inline alphabetical tag add/remove/Refresh controls, Publication Copy/Edit/Refresh/Verify/Other-from-publication, DOI Copy/Edit/Refresh/Verify/No DOI, domains, custom attributes, rendered Markdown summaries/citations/bibliographies, rich Markdown summary with Validate exemplar state and repeated generated-time markers for summaries longer than four paragraphs, Markdown-backed rich APA citation and Bibliography editing with bold/italic typing controls plus rich clipboard copy for APA and bibliography text, manual Publication/DOI/APA/Bibliography verification including confirmed empty Bibliographies with confirmation-gated edit/refresh unverify behavior, confirmation-gated edit/refresh unvalidation for validated summaries, extracted-text cleanup, a document Lock toggle that blocks edits, refreshes, Trash, and property changes without changing verified-field display, duplicate visibility plus side-by-side duplicate resolution or false-positive dismissal from Library, and complete correction history with Restore as Current
...
Valkey stores only derived, rebuildable API payloads and counters. PostgreSQL remains authoritative for documents, search, jobs, history, evidence, auth, and backups. Cache keys include PostgreSQL `cache_revisions`, so manual Refresh Cache and relevant committed writes make older payloads unreachable even if old Valkey keys remain until TTL/LRU eviction. Backend startup hydrates the cache by default through the same Hydrate Cache path, preloading the current revision's deterministic Library pages, saved-search pages, organization/status/dashboard payloads, and visible document details from live PostgreSQL data without turning Valkey into a second database. `MEDUSA_CACHE_HYDRATE_MAX_DOCUMENTS=0` means all visible Library document details; positive values cap detail hydration, and large payloads still obey `MEDUSA_CACHE_MAX_PAYLOAD_BYTES`. The default Valkey memory limit is 8 GB. Settings > Preferences can change the saved Valkey Memory Limit and apply it to the running Valkey service; `MEDUSA_VALKEY_MAXMEMORY` remains the startup fallback. The profile menu shows cache utilization at a glance with a mini graph, and Status includes a Valkey resource monitor for used memory versus limit, peak memory, RSS, keys, evictions, expiry, connected clients, and ops/sec.
...
Release and maintenance refreshes are host-agent compatible. The backend reads `data/deploy/release-status.json` and `data/deploy/release-history.json`, and writes `data/deploy/release-request.json` when the user clicks the header `Upgrade Now` button; Utilities and Status can also write `release-check-request.json` and `maintenance-request.json` for on-demand checks or safe maintenance. If the status file's running build is stale after a container rebuild, the backend uses its current runtime build identity for browser reload decisions so `Reload Now` cannot loop on old JSON. The browser tab that starts an upgrade warns about unsaved edits, locks the UI while the host agent works, polls release status through restart gaps, verifies both `/api/health` and the exact app-shell route that will be reloaded after completion, then cache-busting reloads into the new browser bundle before returning control. All signed-in tabs also show the blocking overlay when a scheduled or user-requested maintenance apply starts. Whenever the current browser bundle is stale relative to the running server build, the header action is labeled `Reload Now`; clicking it performs the same health and app-shell readiness checks before reloading instead of requesting another server update. Same-hash runtime refreshes naturally do not prompt because the embedded browser build stamp still matches the server. The server-side `scripts/medusa-release-agent.py` checks upstream git state, refuses dirty checkouts, verifies non-empty HAProxy cert/key files before any Compose restart, classifies dependency/runtime risk, fast-forwards only for approved upgrade paths, persists explicit build identity variables into ignored `.env`, runs the policy-based verified backup gate only for database/runtime-sensitive changes, rebuilds Compose with those variables and any configured Compose override files, can perform same-tag refreshes with `--pull always`, appends release-history entries after successful fast-forward applies, and updates the status file only after the backend health endpoint and GUI shell route pass through the public proxy. Its health probes resolve `MEDUSA_PUBLIC_HOST` to `MEDUSA_RELEASE_HEALTHCHECK_IP` when set, otherwise to `MEDUSA_BIND_IP`, then `MEDUSA_BIND_IPV6`, then localhost. Template systemd units under `deploy/systemd/` can own the Compose stack and wire the release checker/apply watcher plus twice-weekly maintenance timer/path on a server such as carrot. The app itself does not run arbitrary git or Docker commands.
