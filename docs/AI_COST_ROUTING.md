...
- Metadata and APA Citation Matching stay on `gpt-5.5`; APA Citation Matching uses high OpenAI reasoning effort by default.
- Summary and Accessory Summary generation default to `gpt-5.4` and are prompted as technical plain-text paragraphs by default, not Markdown outline summaries or bibliographic lead-ins; endings should include main takeaways and continued-reading directions.
- Tag suggestion extraction defaults to `gpt-5.4-mini`. The legacy internal task key remains `keywords_topics`, but extracted concepts are stored, displayed, searched, exported, and optimized as flat tags. The same saved Tag Suggestions model preference is used for import tag creation and Optimize taxonomy cleanup, so selecting `gpt-5.4` intentionally moves both routes to that model. Optimize caps broad LLM cleanup prompts to a ranked high-yield subset for latency and token control, while deterministic cleanup still reviews the full scope. Prompts include a compact manifest of existing canonical/candidate tags and prefer matching existing labels before proposing new concise labels. Model output is candidate evidence; import and Concordance run it through tag-governance scoring with alias memory, deterministic similarity, optional cached embedding similarity, library/relationship context, and three-axis relevance/fit/novelty scoring before attaching tags.
- DOI regex plus Crossref DOI/title/author/year matching runs before APA generation; DOI/Crossref evidence is included in the compact GPT APA Citation Matching prompt, APA checking runs with the configured OpenAI reasoning effort (`MEDUSA_OPENAI_APA_REASONING_EFFORT`, default `high`), confident DOI-specific page ranges or article numbers may be completed by the APA model, numeric page ranges are normalized with en dashes, and deterministic formatting remains the fallback for missing or malformed model output.
- Bibliography Cleanup can opt into GPT-5-family OpenAI reasoning through `MEDUSA_OPENAI_BIBLIOGRAPHY_REASONING_EFFORT`, but the default is `off` because large reference lists can be expensive.
...
| Metadata and citation identity | GPT-5.5 metadata extraction with PDF context when size-safe, plus GPT-5.5 APA Citation Matching with high reasoning effort for paired reference-list/in-text output | Local DOI regex plus Crossref DOI/title/author/year matching supplies compact evidence; deterministic APA formatting handles missing or malformed model output | Stronger selected APA model only for high-value ambiguous cases | Ask GPT for the APA pair when DOI/Crossref evidence is present, include known title/authors/year/venue so DOI mismatches stay reviewable, and allow confident DOI-specific page ranges/article numbers with APA en dashes. |
| Summary | GPT-5.4 text-only paragraph-style summary | GPT-5.5 only for selected high-value reruns | Gemini/Claude alternatives only after quality evaluation | Avoid attaching the PDF for routine summaries. Default output should start with substantive analysis, cover original ideas/questions/insights/context, end with main takeaways and continued-reading research directions, and avoid bibliographic lead-ins, bold, italics, bullets, standalone headings, em dashes, or fancy quotes unless explicitly requested. |
...
| DOI and source verification | DOI regex plus Crossref DOI/title/author/year matching, with GPT APA Citation Matching checking the compact DOI evidence against known document metadata | Semantic Scholar, OpenAlex, DOI.org, publisher/static source evidence | Stronger selected APA model only to adjudicate ambiguity from a compact evidence packet | Verified status requires evidence consistency or explicit user acceptance; model-reported DOI conflicts stay in review. |
...
| Retroactive refreshes | Concordance Runs with provider/model and reasoning effort recorded per capability version | Batch or Flex-style cloud processing for slow non-urgent refreshes | Premium model or opt-in Bibliography Cleanup reasoning only for selected scope or failed cheap/local pass | Large library refreshes should default to async discounted processing. |
