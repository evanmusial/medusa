...
- Metadata and APA Citation Matching stay on `gpt-5.5`.
...
- DOI regex plus Crossref DOI/title/author/year matching runs before APA generation; DOI/Crossref evidence is included in the compact GPT APA Citation Matching prompt and deterministic formatting remains the fallback for missing or malformed model output.
...
| Metadata and citation identity | GPT-5.5 metadata extraction with PDF context when size-safe, plus GPT-5.5 APA Citation Matching for paired reference-list/in-text output | Local DOI regex plus Crossref DOI/title/author/year matching supplies compact evidence; deterministic APA formatting handles missing or malformed model output | Stronger selected APA model only for high-value ambiguous cases | Ask GPT for the APA pair when DOI/Crossref evidence is present, but include known title/authors/year/venue so DOI mismatches stay reviewable. |
