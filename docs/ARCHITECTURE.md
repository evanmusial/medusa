...
- Fixed top header with the Medusa emblem, global search, a reserved active-work progress control, optional release upgrade action, theme toggle, and session action. The active-work slot keeps its width while idle so the header does not jump when imports, Concordance, citation refresh work, or release prompts start, but it must shrink and ellipsize before it can collide with the global search field under browser zoom. The release action appears as a compact accent-colored `Upgrade Now` button to the left of the day/night theme switch when a host-side status check reports newer pushed code or when the running server build is newer than the browser bundle; it confirms before reloading and warns that unsaved edits will be discarded.
...
- Domains view is the domain-tree management surface. It provides a searchable alphabetized tree, top-level or nested domain creation, selected-domain editing for name, parent, description, tag associations, and color, child creation, soft delete with confirmation, and a direct list of documents assigned to the selected domain. Domain descriptions and associated tags are domain-level classification hints for future filing suggestions; they do not silently expand assigned-document search text. Top-level domains and every nested sibling group are displayed alphabetically by name.
...
- Release detection is file-backed so the web backend does not need host Docker or git control. `MEDUSA_RELEASE_STATUS_PATH` defaults to `/app/data/deploy/release-status.json`, and `MEDUSA_RELEASE_REQUEST_PATH` defaults to `/app/data/deploy/release-request.json`. A host-side release agent such as `scripts/medusa-release-agent.py` owns `git fetch`, fast-forward-only merge, Compose rebuild/restart, and health verification. The backend reads the status file and writes a request file when an authenticated user clicks `Upgrade Now`; a server timer/path unit can consume that request outside the container.
