...
- Preserve original files and process them into searchable metadata, text, summaries, cropped graphic assets, figures, formulas, publication/container links, and citation candidates.
...
Typography should reserve compact, heavier treatment for controls, interaction chips, and button-adjacent metadata. Section subtitles, metric labels, table headers, usage ledgers, and ordinary panel copy should use sentence/title case or source text casing with moderate weight so repeated operational surfaces remain readable. Normal UI text should not drop below 14px on operational workbenches, shell popovers, menus, or status readouts; smaller treatment should be avoided unless it is icon-only-adjacent decoration that remains legible under browser zoom.
...
- Fixed top header with the Medusa emblem, a capped-width global search field with a right-side clear action that exits filtered search mode by clearing the query, a reserved active-work progress control, optional release upgrade action, icon-only Status action, and a far-right user options menu. The user menu exposes larger Valkey cache usage/hit-rate status with a mini utilization graph, Refresh Cache, Hydrate Cache, Quench Cache, a discoverable divided Fast Finder entry, Day/Night mode, Settings, and Sign out, replacing the former dedicated logout and theme icons. Refresh Cache, Hydrate Cache, and Quench Cache show an animated full-row in-flight fill plus `1%` through `100%` copy in the menu row; the Hydrate Cache row intentionally stays compact and does not include the long hydration payload explanation. Hydrate Cache uses backend-reported hydration progress for startup and manual runs when available, falling back to local action feedback only before worker status arrives. Quench Cache empties resident Valkey cache entries without bumping PostgreSQL cache revisions or changing source data, so the next request can rebuild payloads from durable state. The same shell-owned action state is mirrored in the header active-work progress slot so cache work remains visible after the menu closes, and that main progress surface may show live completed/planned payload counts, planned document counts, cache writes, already-hot payloads, bypasses, and errors so percentage plateaus still show observable work. When the active-work slot represents controllable cache hydration, queued import work, or queued Concordance work, it exposes compact Pause/Resume and Stop controls; Stop asks for Medusa-owned confirmation and never silently deletes ready Library documents. The app-wide command palette opens with `Cmd K`/`Ctrl K` and can jump to workspaces, saved searches, and common shell actions such as clearing search, cache refresh, hydration, or quenching without adding a second navigation rail. The active-work slot keeps its width while idle so the header does not jump when imports, Concordance, citation refresh work, cache refresh/hydration/quench, or release prompts start, but it must shrink and ellipsize before it can collide with the capped global search field under browser zoom. Completed import notices are idle-state receipts only: active imports, Concordance, Inquests, backups, cache work, or release work take priority in the header and suppress stale completion receipts until no active work remains. The release action appears as the same compact accent-colored `Upgrade Now` button when a host-side status check reports newer pushed code, and switches to `Reload Now` whenever the server is already running a newer build than the current browser bundle. Same-hash runtime refreshes do not prompt because the embedded browser build stamp still matches the server. The icon-only Status action opens `/status` and has a quick hover summary for uptime, memory, database, and active work; the full page shows build/version identity with the short commit hash, uptime, memory and disk footprints, database size, Valkey cache state, runtime versions, proxy state, storage path details, backend image identity, hot-route p95 timings, active queue age/depth, database relation footprints, and storage-footprint splits from existing authenticated status APIs. Its final section is Library Fun Stats: visible-library document/page/figure counts, bibliography reference totals, parsed and indexed word/character counts, author/DOI/citation coverage, project-source use, notes, annotations, domains, and tags. Docker image/layer sizing remains optional when the Docker Engine socket has been deliberately mounted; without that socket, the Status and Utilities backend-image cards fall back to the backend base-image identity reported by runtime inventory and state that layer sizing is disabled. Starting an upgrade warns that unsaved edits will be discarded, locks the initiating browser tab behind a release progress overlay, waits for the host agent to report completion and for both `/api/health` and the exact app shell route to answer through the public proxy, then reloads into the new browser bundle before returning control. If a browser is already stale when the user clicks `Reload Now`, the same health and app-shell checks run before the cache-busting reload begins. Ordinary API calls share a backend-availability signal: network failures, 502/503/504 proxy responses, and explicit backend restarts lock the app behind a reconnecting overlay, broadcast that state to other same-origin tabs, and poll `/api/health` every second until Medusa recovers; otherwise a lightweight five-second visible-tab health watchdog keeps rebuild/restart outages from staying invisible behind slower workspace polling. The React startup screen and HAProxy maintenance fallback use the Medusa emblem plus the lowercase Century Schoolbook `medusa` wordmark, with animated SVG snake strokes anchored into the emblem hair and a few heads reaching slightly beyond the mark while the canonical emblem asset stays unchanged; the HAProxy fallback keeps the animation visible for about five seconds during startup or restart even when health checks recover quickly.
- Manual `Upgrade Now` approval does not treat the approving browser's active session as a blocker because the upgrade transaction immediately locks and reloads that tab, but active imports, Concordance jobs, Inquests, backups/restores, database maintenance, or host-side release phases still disable the action.
- A hidden `/startup-animation` route exists for frontend-only animation testing. It bypasses authenticated app data loading and renders the same emblem, lowercase Century Schoolbook wordmark, and head-anchored snake overlay so wait-screen motion experiments can be inspected without waiting for backend health.
- Active in-app release, reload, and backend-reconnect overlays reuse the same animated Medusa emblem and lowercase wordmark lockup as the startup animation route, while failed release states keep a warning icon so operational errors remain visually distinct.
...
- Queue, Import, Settings, Utilities, and Finances share one operational workbench language: full-width open sections separated by quiet horizontal rules, ordinary copy at roughly 15px, 44px form controls, section headings with restrained icon treatment, and fact readouts arranged as row/column separators instead of individually boxed stat cards. Bounded controls such as dropzones, charts, dialogs, and dense editable preset groups may still have a subtle frame when the boundary supports interaction, but ordinary settings, maintenance facts, usage numbers, runtime facts, backup status, import estimates, and finance metrics should not look like separate dashboard tiles.
- The app shell treats idle resource use as a first-class UX constraint. Header and navigation badges come from compact dashboard counts instead of whole-list polling, heavy workspace datasets are fetched only when their workspace is visible, search input waits briefly before refetching document results, idle document-list results stay fresh briefly to avoid redundant view-switch fetches, and fast polling is reserved for active imports, Concordance, backups, cache refresh/hydration, or release upgrades. Detailed import-job rows are hydrated only on Import, Queue, Activity, Health, and Portfolio work surfaces rather than whenever a dashboard queue count is nonzero; header progress and navigation badges must remain satisfied by `/api/dashboard`. Detailed Concordance APIs must return all queued/running/paused runs and jobs while capping only completed history, so list windows cannot hide in-flight work or undercount active background jobs. Queue cost-estimate helpers batch model-pricing lookups across usage rows so one queue refresh does not repeatedly scan pricing history while imports are in flight. The Library uses `/api/documents/list` result windows with total counts, a 50-row default, Settings-persisted page sizes with suggested values of 100/200/300/400/500 and arbitrary whole-number sizes of 10 or greater, and virtualized rows instead of rendering the full corpus in the browser by default. List responses are deliberately slim and omit heavy full-text, full summaries, bibliography, and citation blobs from row queries; row summaries are bounded previews and row tags/domains/projects use compact id/name-style references while full records remain available through detail and organization endpoints. When a bookmarked document route opens Library, the list and tag data get the first render opportunity before the heavier selected-document detail payload is requested. Secondary workspaces can still use the shared document-list API without Library-only enrichments such as duplicate badges and project membership when they only need a lean all-document reference set.
...
- Main Library view uses a tri-pane layout:
  - The reserved Tags `breakfix` and `replace` have special visual treatment: Library rows render them as red and green badges respectively, and preview/Reader TAGS chips use the same tint while remaining normal removable tag chips.
...
- Resizable right document detail/correction pane for authenticated original PDF preview, normalized one-page parsed text reading and editing, color-tinted domain chips listed before inline alphabetical TAGS chips with remove controls, a separate domain Assign picker row that lists all domains with counts, an end-of-list add field, and a Tag Refresh action, Publication Copy/Edit/Refresh/Verify/Other-from-publication controls, DOI Copy/Edit/Refresh/Verify/No DOI controls, APA Reference List and APA In-Text Citation Copy/Edit/Refresh/Verify sections, generated Summary Copy/Edit/Refresh/Validate controls, Bibliography Copy/Edit/Refresh/Verify controls, Inquests, extracted figures with relabel/description correction and delete controls, attributes, compact history rows with version stepping and Restore as Current, evidence, Composition, a lock toggle before Trash, and a single-document Trash control. The detail/Reader top identity strip is sticky and user-configurable from Settings; Title and Authors are sticky by default, with optional Year, DOI, Priority, and status chips. Preview actions are grouped by purpose: Reader/Close Reader plus compact icon-only Edit, Link, Download Original, and Lock controls; Ask, Related, Composition, Concord, and Formulas as document work actions; and confirmation-gated Trash as the destructive action. `Open Original` is intentionally omitted because authenticated Download Original and in-app PDF preview cover the durable original access paths without another competing button. Detail responses keep history row metadata, changed-field chips, restorable flags, and preview hints lightweight; full before/after `DocumentVersion` snapshots remain stored server-side and are used by the restore endpoint rather than being sent on every document click. Clicking Edit for document metadata reveals the correction form and focuses the Title field. Publication controls show the primary journal, book, textbook, proceedings, report series, or other container plus appearance details such as volume, issue, pages, article number, edition, chapter, series, event, identifiers, source, model, and verification state; Other from publication applies the Library `publication_id` filter. DOI keeps its compact value-chip styling; DOI Refresh queues citation refresh so DOI/Crossref discovery can run even when no DOI is currently stored, Edit lets the user provide a DOI manually, and No DOI records a manual metadata-evidence flag without storing a placeholder DOI string. Citation sections show Copy, Edit, Refresh, Verify, compact model/provenance controls that display the model name or `user provided` without repeating field-label text, and Markdown-backed rich editors with bold/italic/remove-format controls plus Cmd/Ctrl-B/I shortcuts in edit mode, so formatting can be typed visually while saving back to Markdown. Summary Edit uses Markdown-oriented rich-text controls for bold, italic, underline, bullet/numbered lists, indentation, and formatting removal while preserving line breaks; Summary Refresh queues a summary-only Concordance run using the selected Summary model; Summary Validate records the current text as the validated exemplar, protects it from unconfirmed edit/refresh replacement, and long summaries with more than four paragraphs repeat their generated time above and below the rendered text. Bibliography shows the relative generation time when extraction provenance is available, supports the same Markdown-backed rich editor with bold/italic formatting and keyboard shortcuts in both the focused Bibliography editor and full correction form, and can be manually marked verified even when empty to affirm that the source document provides no reference list. Verified Publication, DOI, APA citation, Bibliography, and Summary validation state is stored with the verifying/validating user, timestamp, and accepted value state or summary digest; editing or refreshing verified Publication/DOI/APA/Bibliography data or validated Summary text requires confirmation, and confirmed edits/refreshes remove the affected verified or validated state until it is set again. Bibliography Refresh queues a forced document-scoped `bibliography_extraction` Concordance run so the current source reference list can be re-extracted from parsed pages/PDF evidence on demand, then the selected Bibliography Cleanup model formats it as APA-sorted Markdown sources, one per line, conforming to the selected reference/source style, with cited work titles in sentence case, journal/proceedings/container titles in title case, personal authors inverted to `Surname, Initials.`, entries sorted by first-author surname, and intelligent grouping and italics preserved; broad unforced Concordance still protects existing bibliography text from silent overwrite. Tag Refresh queues a forced `tag_refresh` Concordance job for the current document, confirms that existing tag assignments will be removed, then reruns import-style Tag Suggestions and governance with existing-tag reuse before creating any new candidate. Default generated document summaries and Inquest answers are prompted as complete-sentence technical paragraphs written at a graduate academic level suitable for a master's-degree reader that open with the document's substantive claim, problem, method, finding, or conceptual contribution instead of author/year/medium/source-title framing; they cover original ideas or concepts, subject-matter areas, research questions raised, conclusions or novel insights, surprising or counterintuitive results when present, academic context, main takeaways near the end, and adjacent research areas or related topics worth pursuing for continued reading, without bold, italics, bullets, em dashes, fancy quotes, standalone headings, single-word openings, or leaked schema metadata unless the user explicitly asks for another format. Inquests are user-prompted document questions that try inline model answers, fall back to durable worker jobs on timeout, and display saved answers inline with optional titles. Composition opens as a near-fullscreen modal with a compact left summary rail for persisted estimate-vs-actual cost comparison, import cost composition, provider spend, local processing time, and processing issues, plus a larger right-side React Flow pipeline workspace whose connected nodes follow import execution order. The normal desktop layout should avoid whole-modal vertical scrolling by spreading content horizontally and reserving most of the available area for the pipeline; narrow viewports may stack and scroll as a fallback. The pipeline includes second-pass import stages such as OCR audit, page normalization, structured-table evidence, visual extraction/context, bibliography extraction, and downstream model work with the models/preset active at processing time; later Concordance Runs append their own capability and model-call nodes instead of being folded into the original import path.
  ...
  - Summary verification is visibly labeled `Verify` / `Verified` in the document detail pane to match the other manually verified document fields, while the persisted evidence key and confirmation-gated protection still use the existing summary-validation contract.
...
- Settings opens with a Processing Capacity section so server import-worker concurrency, the disabled-by-default Cloud Run worker-pool profile, and Slipstream remote workers are visible together before storage/account details. The Cloud Run panel exposes enablement, numeric target concurrency, a CPU/memory flavor dropdown, active Cloud Run clients/leases, generated `gcloud run worker-pools` commands, and typical runtime cost estimates. The Slipstream panel shows enablement status, one-time enrollment tokens, registered clients, capability/capacity metadata, last check-in, active leases, and disable/revoke/cancel controls. Queue and Activity rows show execution-location and next-stage labels for import and Concordance jobs, using the same lease heartbeat/expiry metadata for assigned local, Slipstream, or Cloud Run work.
...
- Interactive controls use app-styled delayed tooltips rather than native browser title bubbles. Buttons, action links, dropdowns, checkboxes, and free-text inputs should explain the action or edit they perform after a 1.25-second hover; disabled controls should keep the action explanation and add the specific reason the control is unavailable.
...
- Optional Slipstream clients: remote Python processes registered with the central app that poll over HTTPS for leased import or Concordance work; they do not expose inbound ports and never connect directly to PostgreSQL. Cloud Run worker pools run the same Slipstream client in `--cloud-run` mode as a default-disabled runner profile.
- `db`: PostgreSQL with `pgvector`.
- `valkey`: internal Valkey cache service using the Redis protocol, no host port, no persistence, bounded by `MEDUSA_VALKEY_MAXMEMORY` at startup, `allkeys-lru` eviction, and attached only to the private backend/worker cache network. The default memory cap is 8 GB, dedicated server examples use 24 GB for the current corpus, and Settings > Preferences can save and apply a different Valkey Memory Limit to the running service.
- `haproxy`: TLS terminator and redirect/proxy on host port `MEDUSA_HAPROXY_PORT` (`3737` by default), with internal stats on `8404`.
...
- `docker-compose.yml` wires the application services and exposes only HAProxy on host port `MEDUSA_HAPROXY_PORT` (`3737` by default); Valkey is not attached to the public-facing proxy/frontend path and has no published host port. The metrics overlay is the only planned host-published exception, and should bind only to the monitoring interface/IP rather than `0.0.0.0`.
- Backend and worker startup initialize PostgreSQL through Alembic migrations before normal app workflows open. Backend startup also schedules a background Valkey cache hydration by default after the saved Valkey memory limit has been applied, so a cleared or recreated cache starts warming as soon as the service is back online without blocking health.
- Dependency freshness is managed through root `renovate.json` plus `docs/DEPENDENCY_UPDATE_PLAN.md`: Renovate checks Docker, backend, and frontend dependency surfaces twice weekly, while critical security updates can be handled immediately. A host-side maintenance lane can also run Tuesday/Friday during the quiet `03:00-06:00 America/Indiana/Indianapolis` window for already-merged safe updates and same-tag image refreshes. Routine restarts, rebuilds, safe app updates, and same-tag image refreshes do not require a database backup. The host agent requires a fresh full PostgreSQL backup only when the classified change touches database schema or persistence, backup/restore tooling, runtime container definitions, non-patch backend runtime dependencies, or major underlying program versions such as PostgreSQL/pgvector. Runtime image updates must preserve explicit tags, the Valkey private cache network, and the invariant that base application Compose publishes only HAProxy for the app path; the optional metrics overlay may additionally publish the bearer-protected metrics sidecar on the configured monitoring bind IP and port. Docker Engine and Compose plugin updates are report-only recommendations outside Medusa automation.

Data storage:

- PostgreSQL is the system of record.
- Valkey is a derived response-cache and operational-counter layer only. It stores rebuildable API payloads and status/counter data, not source-of-truth documents, search indexes, jobs, correction history, auth state, or evidence. Valkey is internal-only, not exported, and not backed up. Cache keys include durable PostgreSQL `cache_revisions` rows so relevant writes and manual refreshes make old Valkey payloads unreachable even if old keys remain until TTL/LRU pruning. The default response-cache TTL is seven days and the default per-payload cap is 32 MiB so larger document detail, workspace, and list payloads remain cacheable when the Valkey memory budget allows it. Detail responses keep page reading/edit text available while omitting duplicate raw/reader page text when normalized text already covers the page, and send only a bounded `Document.search_text` preview because full search text duplicates indexed/page content and the frontend uses it only as recommendation evidence. Startup and manual cache hydration fill the current revision's safe PostgreSQL-derived JSON payloads broadly, including dashboard/status/preferences, organization chrome, publications, all deterministic Library filter/sort pages, saved-search pages, visible document details, document composition/annotations/events/notes, project/Recon/Portfolio workspace payloads, review queue, finance summaries, backup runs/estimate, and Concordance/job surfaces; hydration first warms the login/Library landing path, including core shell chrome and the default title-sorted first Library page for both the saved and default page sizes, before moving into broad filter and detail sweeps. Hydration avoids binary downloads, auth/session routes, live host status routes, and GET routes that perform external lookups or hidden writes. `MEDUSA_CACHE_STARTUP_HYDRATE=true` is the default and should remain enabled for normal deployments so a cold cache warms immediately after restart; turning it off is an incident-only mitigation, not a steady-state production setting. `MEDUSA_CACHE_HYDRATE_MAX_DOCUMENTS=0` means all visible Library document details plus their adjacent document payloads, and `MEDUSA_CACHE_HYDRATE_PAGE_SIZE` controls list-page hydration windows. The Valkey memory cap defaults to 8 GB and is configurable from Settings, which persists the value and applies it through Valkey's runtime configuration path when reachable.
...
- The backend storage adapter reuses the active GCS/local storage service across requests and refreshes it when the bucket, prefix, credential path, or credential file metadata changes. Authenticated asset routes should not recreate Google credentials and clients for every thumbnail or page request.
...
- Processing/document cache lives under `data/processing-cache`, is ignored by git, and keeps local PDF copies for staged/queued/running/paused/failed work plus recently completed imports within the configured document cache budget. For HTML/text imports, this cache contains the generated PDF mezzanine. Clearing staged uploads removes their managed cache files before deleting the staged queue records.
...
- `backend/app/services/slipstream.py`: remote-processing enrollment, request-signature verification, enrollment capability/capacity limits, lease coordination shared by local, Slipstream, and Cloud Run workers, artifact access, heartbeat/expiry, import-preprocess result application, Cloud Run status/scale-plan helpers, Cloud Run runtime cost formulas, and result-application helpers.
- `backend/app/slipstream/client.py`: bundled outbound Slipstream runner for import preprocessing, with signed polling, concurrent lease workers, heartbeat threads, local state/key storage, one-at-a-time claim ramping, empty/error claim backoff, transient check-in retry behavior, Cloud Run Secret Manager state loading, raw-text extraction, partial result manifests, Cloud Run runtime Composition rows, and best-effort CPU affinity/QoS hints for local laptop operation.
...
- `backend/app/services/verifier.py`: DOI/source-link resolution helpers, Crossref lookup, Crossref bibliographic query matching, OpenAlex/Semantic Scholar/DataCite/OpenCitations Meta/Europe PMC title DOI discovery, DOI.org Registration Agency checks, targeted title web-search DOI fallback, local DOI/source evidence scoring, and verification helpers.
- `backend/app/services/publications.py`: publication/container normalization, provider/model candidate reconciliation, Publication and DocumentPublication upsert/matching, citation/search enrichment, and verification helpers.
...
- Header progress shows imports first when imports are active, but the import progress bar reflects the active job's processing step, such as page-normalization progress, rather than treating the currently running file as fully complete. When imports are active alongside Concordance or Inquest work, the header detail line still includes the other active-work counts so background jobs do not disappear behind the import. Otherwise the header shows a single active Concordance/citation/Inquest background run or an aggregate active-count summary across multiple background runs. It is hidden but width-preserving when idle and opens Queue when clicked.
...
- Settings > Preferences includes a browser-local Button Motion gallery for comparing shared async button and dialog feedback treatments. The default `Rail Only` style keeps non-circular busy glyphs still while preserving rotation for refresh/reload-style icons, and alternate choices move attention to a surface sheen, control rim, halo, beacon dot, status meter, surface pulse, or static state marker without changing backend job semantics.
- Motion for React is the standard runtime animation layer for restrained cockpit motion. Focused vertical reading/work panes such as Library rows, document detail, Reader PDF/text, and compare-mode panes show thin accent-colored scroll progress rails at the top of the scroll area. Single-purpose buttons may use small hover/tap gestures that do not change layout, while async buttons keep the existing progress/feedback language. First-load or buffered content can use fixed-footprint shimmer skeletons where the content shape is known, except for the main Library result rows, which should avoid skeleton redraws and keep cached rows visible whenever possible. Medusa-owned dialogs, command surfaces, tabs, and chips may use short opacity/scale or selected-marker transitions, all honoring `prefers-reduced-motion`.
- The Library Publication Refresh action queues a forced `publication_metadata` Concordance Run for the current document and can require confirmation before clearing a verified publication link. The Library DOI and citation Refresh actions queue a forced `citation_refresh` Concordance Run for the current document. DOI Refresh remains available when DOI is missing so stored text and Crossref/title evidence can be searched; APA Reference List and APA In-Text Citation buttons start the same durable pair refresh because both citation surfaces are generated from the same citation metadata/model preference, validated together, and displayed with shared in-flight/success/error feedback. Summary Refresh queues a forced `summary_refresh` Concordance Run for the current document and uses only the selected Summary model; if the current summary is validated, the detail action requires confirmation before clearing validation and queueing replacement, while generic Concordance summary refresh work skips validated summaries. Tag Refresh queues a forced `tag_refresh` Concordance Run for the current document and uses only the selected Tag Suggestions model plus the import-style tag governance scorer. If the user stays on the document pane, the button-level watcher can flash completion/failure; success flashes fade quickly while error flashes remain visible longer. If the user navigates away, the app shell still follows the durable run and displays terminal state.
...
- Import jobs remain represented by Queue rows and the header active-work progress control because their progress is dashboard-backed; retry, bulk retry, and clear controls use the same transient button-feedback convention. Queue also shows non-complete Concordance jobs in the same operational surface so active Concordance work remains visible while imports are running.
...
- `Publication`, `PublicationAlias`, and `DocumentPublication`: first-class publication/container identity and per-document appearance rows. `Publication` stores normalized title, type, publisher/imprint, ISSN/ISSN-L, ISBN, container DOI, source URL, provider ids, aliases, metadata, and evidence. `DocumentPublication` stores the document's role in that publication, appearance type, volume/issue/pages/article number, published date/year, edition/chapter/section, series/proceedings/event metadata, source, model, confidence, verification state, and evidence. `Document.journal` and `Document.publisher` remain legacy/citation display mirrors of the primary publication title and publisher.
...
- `SlipstreamClient`, `SlipstreamEnrollment`, `SlipstreamLease`: remote-processing registration and quorum state. Enrollments store only one-time token hashes plus expiry/use state, allowed capabilities, and maximum capacity. Clients store public keys, server-clamped capability/capacity metadata, status, revocation state, last check-in, runner metadata, and recent nonce replay data. Leases store job type/id, assigned worker kind/client, lease token hash, heartbeat/expiry/completion state, payload/idempotency metadata, work kind/result mode, and terminal error/cancel details. A partial unique active-lease index over `(job_type, job_id)` enforces one active owner for a job across local and Slipstream remote workers.
...
- Publication/container evidence is reconciled separately from the document's own title/authors/DOI. Matching prefers exact provider source ids, container DOI, ISSN/ISSN-L, ISBN, then normalized title with compatible type/publisher. Crossref/DOI evidence wins over OpenAlex/Semantic Scholar/book-provider evidence, which wins over GPT extraction; GPT fills gaps but does not override stronger provider evidence. Conflicts remain in `DocumentPublication.evidence` and unresolved or model-only links stay reviewable. Existing documents are not automatically backfilled by migrations or broad defaults; publication rows are created at import, by explicit document refresh, or by an opt-in `publication_metadata` Concordance scope.
- Import enrichment and `citation_refresh` run DOI/source-link resolution before APA formatting. The local pass scores document metadata, extracted text, and the extracted Bibliography field for title-supported DOI candidates and stable source URLs; it rejects unrelated reference-list DOI noise unless the candidate context strongly matches the current document. The network DOI ladder then uses Crossref title lookup plus Crossref bibliographic query matching, Semantic Scholar, OpenAlex, DataCite, Europe PMC, OpenCitations Meta, DOI.org Registration Agency checks, and finally targeted title web-search evidence before giving up.
- Resolution evidence is stored under `Document.metadata_evidence["doi_source_resolution"]`, `["doi_discovery"]`, and `["source_link_resolution"]`, including attempted sources, compact candidates, selected DOI/source URL, conflicts, confidence, and Registration Agency evidence when available. Queue citation candidates include the same resolution evidence inside their source metadata so uncertain DOI/source decisions can be inspected without inventing a separate review table.
- APA citations favor DOI links whenever a DOI can be located and verified. If no DOI can be verified, the citation path fills `Document.source_url` from the best direct stable source link it can support, preferring PDFs or other static documents over transient search/result pages, and deterministic APA formatting appends that source URL.
...
11. The user presses Process Uploads to promote all staged import jobs to `queued`; local workers and enabled Slipstream clients claim queued jobs through the shared lease coordinator. Local workers can complete the full import pipeline. The bundled remote runner claims only `import_preprocess` work, downloads the original PDF, extracts raw page text with the configured raw-text extractor, submits page/search/composition evidence, and returns the job to `queued` at `normalizing_pages` so the central worker owns enrichment, indexing, model calls, citation/tag logic, storage, and final completion. Import processing defaults to 4 concurrent local jobs from one worker process and can be changed to any positive value in Settings; Slipstream clients additionally enforce their server-registered capacity.
...
19. OpenAI metadata extraction runs only when `OPENAI_API_KEY` exists; otherwise a low-confidence review record is produced. Metadata extraction asks for visible authors, affiliations, normalized contact emails, and structured publication/container candidates with evidence snippets and page hints. The import path reconciles those candidates with DOI/Crossref evidence first, then bounded OpenAlex/Semantic Scholar and book-provider evidence where available, and stores the result in `Publication` plus `DocumentPublication` without treating GPT output as verified by itself.
20. Extraction and async document-intelligence work are split into Settings-selectable tasks: Raw Text Extraction, Metadata, Summary, APA Citation Matching, Tag Suggestions, Text on Pages (Normalization), Bibliography Cleanup, Formula Capture, Text Chunk Encoding, Inquests, Recon, and Portfolio Assessment. The compatible internal key for Tag Suggestions remains `keywords_topics`, but persisted output is flattened into tags. Metadata, publication detection, and APA fallback matching default to `OPENAI_MODEL=gpt-5.5`; APA Citation Matching uses `MEDUSA_OPENAI_APA_REASONING_EFFORT=high` by default; Summary defaults to `gpt-5.4`; Tag Suggestions defaults to `gpt-5.4-mini`; Bibliography Cleanup defaults to `gpt-5-mini`, has an expensive off-by-default `MEDUSA_OPENAI_BIBLIOGRAPHY_REASONING_EFFORT` option for GPT-5-family hard cases, runs a targeted author-preserving repair retry when cleanup drops visible authors, then retries remaining unsafe cleanup once with `gpt-5.4-mini`, and is used for forced/ad hoc Bibliography Refresh rather than default import; Formula Capture defaults to `gpt-5.4` and is manual-only through Concordance; Inquests defaults to `gpt-5.4`; Recon defaults to the current Inquests-style synthesis model; Portfolio Assessment defaults to `DEFAULT_GPT_MODEL` currently `gpt-5.5`; Text Chunk Encoding defaults to `OPENAI_EMBEDDING_MODEL` and offers `text-embedding-3-small`, `text-embedding-3-large`, and `text-embedding-ada-002`.
21. By default, document intelligence is routed by task. Metadata extraction may send the original PDF as a Responses API file input when `MEDUSA_OPENAI_SEND_PDF=true` and the file is below `MEDUSA_OPENAI_PDF_FILE_MAX_MB`; if that PDF-backed metadata call hits a provider context-window error, the import retries metadata extraction once with extracted text only and records the fallback in metadata evidence instead of failing the import. Summary and tag extraction use extracted text only. `MEDUSA_OPENAI_COMBINE_DOCUMENT_INTELLIGENCE=true` is an opt-in legacy mode that runs Metadata, Summary, APA Citation Matching, and Tag Suggestions as one structured `core_document_intelligence` Responses call using the Metadata model selection.
...
- Local and Slipstream remote claims create `SlipstreamLease` rows so one coordinator owns the running transition. PostgreSQL is the quorum authority: a partial unique active-lease constraint over `(job_type, job_id)` prevents double assignment, while `worker_kind` distinguishes local workers from remote clients. Remote leases heartbeat every `MEDUSA_SLIPSTREAM_HEARTBEAT_SECONDS` seconds by convention and expire after `MEDUSA_SLIPSTREAM_LEASE_TTL_SECONDS`; expiry returns the job to `queued`, clears the job lock, records a processing event, and allows another client or local worker to claim it. Active leases attached to jobs that are no longer `running` are also expired so stale coordination rows cannot block work. Late results from expired/released leases are rejected unless they are duplicate submissions for the already accepted lease idempotency key.
- Slipstream remote clients authenticate with Ed25519 signed requests, never receive PostgreSQL, OpenAI, Gemini, Google Vision, GCS, or database credentials, and currently use the main backend HTTPS endpoint as the control plane. Cloud Run workers follow the same secret boundary and read only their registered Slipstream `client_id` and private key from Secret Manager. A dedicated IP/port service remains unnecessary until polling/result traffic or isolation needs justify a separate service boundary.
...
- After a released import queue drains and there are no queued/running/paused import jobs left, the worker runs PostgreSQL `VACUUM (ANALYZE)` across all tables when Postgres is the active backend. Staged jobs do not count as active import processing for this drain check.
...
- `page_text_normalization` v4: conforms raw extracted page text into standard readable paragraph flow using OpenAI when configured and local cleanup as a fallback; it preserves headings, labels, captions, citations, equations, lists, tables, front-matter bands, and reading flow across columns/graphics without converting graphics to Markdown. Concordance reruns use the original PDF context when available, and this generation accounts for first-page publisher layouts where article-info/abstract regions sit above the introduction in separate visual bands.
...
- `citation_refresh` v4: regenerates Markdown APA 7 reference-list text with sentence-case cited work titles and title-case containers plus APA parenthetical in-text text by gathering DOI/source-link evidence first, filling missing fields, and sending the selected APA Citation model a compact packet of resolution evidence, known title/authors/year/venue metadata, excerpts, and the configured APA reasoning effort so it can format the pair, complete confident DOI-specific fields such as page ranges or article numbers, and flag DOI mismatches. Missing DOI discovery uses local DOI/context scoring, Crossref bibliographic query matching, Semantic Scholar, OpenAlex, DataCite, Europe PMC, OpenCitations Meta, DOI.org Registration Agency checks, and targeted title web-search evidence before retrying Crossref by any selected DOI. If no DOI verifies, the refresh can use a high-confidence direct source URL, preferably a PDF/static source. The paired APA output is validated, numeric page ranges are normalized with en dashes, malformed or missing model output falls back to deterministic metadata formatting without another model call, resolution conflicts keep the citation in Queue review, and model/provenance are recorded.
...
- GCS service accounts must be able to create original PDFs and extracted assets, read them back for preview/export/reprocessing, and delete temporary smoke-test objects when verifying credentials. When CDN-backed asset cleanup is enabled, the credential used by the worker also needs permission to invalidate the configured global URL map, or storage deletion will still complete while CDN invalidation remains retryable/reportable.
- Do not track API keys, service-account JSON, or generated data.

Network:

- The app listens externally through the HAProxy service. `MEDUSA_HAPROXY_PORT` controls the origin/listener port and defaults to `3737`; `MEDUSA_PUBLIC_PORT` controls the browser-facing HTTPS port used in redirects, forwarded headers, status output, and generated public URLs. Local and LAN deployments normally leave both values at `3737`; Cloudflare-proxied deployments can use `MEDUSA_PUBLIC_PORT=443` while an Origin Rule routes the hostname back to origin port `3737`.
- HAProxy terminates TLS for `MEDUSA_PUBLIC_HOST`, redirects plain HTTP requests on the same origin listener to `https://MEDUSA_PUBLIC_HOST` plus the configured public port suffix, routes `/api/*` to the internal backend service on `backend:8000`, and proxies browser shell/assets to the internal frontend service on `frontend:3737`. The HAProxy app and metrics server lines use the Docker DNS resolver with no stale initial address so release/maintenance container recreation cannot leave the public proxy pinned to an old backend, frontend, or metrics container IP.
- Durable GCS-backed research assets are fronted by a separate HTTPS asset host such as `assets.medusa.evan.engineer`. For GCS buckets this is a global external Application Load Balancer with a backend bucket and Cloud CDN enabled, not a Network Load Balancer. `MEDUSA_ASSET_CDN_BASE_URL` turns on CDN redirects for authenticated original PDFs, figure assets, and any stored page-image objects; `MEDUSA_ASSET_CDN_SIGNED_URL_KEY_NAME` plus `MEDUSA_ASSET_CDN_SIGNED_URL_KEY` preserve the current private-asset posture through Cloud CDN signed URLs instead of public bucket objects. The Cloud CDN cache-fill service account for the project must have `storage.objectViewer` on the bucket so private GCS objects can be fetched only after a valid signed CDN request. Future durable asset work should store canonical `gs://` storage URIs in PostgreSQL and issue signed `assets.medusa.evan.engineer` URLs through authenticated app gates; do not add new browser-facing local-origin asset URLs except as an explicit no-CDN fallback. Do not serialize expiring signed CDN URLs into Valkey-cached document-detail payloads; use authenticated routes as authorization gates that issue short-lived CDN redirects. A DOM `img.src` or link may still show the authenticated `/api/...` gate; CDN verification must follow the redirect chain and confirm the final `Location` host plus CDN response, not inspect only the initial URL.
...
- `MEDUSA_SLIPSTREAM_ENABLED` gates remote work claims. `MEDUSA_SLIPSTREAM_PUBLIC_BASE_URL` lets work bundles return absolute artifact URLs. `MEDUSA_SLIPSTREAM_LEASE_TTL_SECONDS`, `MEDUSA_SLIPSTREAM_HEARTBEAT_SECONDS`, `MEDUSA_SLIPSTREAM_MAX_RESULT_MB`, `MEDUSA_SLIPSTREAM_REQUIRE_TLS`, and `MEDUSA_SLIPSTREAM_SIGNATURE_WINDOW_SECONDS` bound remote lease recovery, check-in expectations, upload size, transport security, and request replay windows. Settings > Slipstream creates one-time enrollment tokens with allowed capabilities and maximum capacity; the server clamps registration and check-in updates to those enrollment limits. `MEDUSA_CLOUD_RUN_WORKERS_ENABLED=false` and the DB Cloud Run preference keep worker pools disabled by default; Cloud Run target concurrency defaults to `1` when enabled, target `0` when disabled, Economy `1 vCPU / 2 GiB` flavor by default, import-only jobs, max `4` instances, and Secret Manager-backed client-state secrets. Additional saved flavors are Balanced `2 vCPU / 4 GiB`, Performance `4 vCPU / 8 GiB`, and High Memory `4 vCPU / 16 GiB`.
...
- Original PDFs are served through authenticated `/api/documents/{document_id}/original` responses and should not require public GCS objects. When the asset CDN is configured, that route redirects to a signed CDN URL so the app authorizes access but does not stream the bytes; adding `download=1` still uses the local Download Naming preference for the attachment disposition when the backend serves the file directly and passes the requested disposition through the CDN URL when possible. The in-app Reader preview renders individual pages through authenticated `/api/documents/{document_id}/pages/{page_number}/image` PNG responses so page-scoped controls can track scroll position and later support geometry hints without depending on browser PDF plugin internals; if a page already has a durable `image_uri`, the route can redirect to the signed CDN asset instead of rendering from the original PDF. Figure asset routes follow the same authenticated-gate-plus-CDN-redirect pattern when configured.
- Deleting an extracted figure removes the `Figure` row and inline reader markers immediately, then enqueues the former asset URI in `asset_deletion_queue` instead of deleting storage synchronously on the request path. The worker checks the queue on startup/idle and then hourly by default, skips deletion if a current document/page/figure row still references the URI, deletes the object through the active storage adapter, and batches CDN invalidation by stored wildcard path when `MEDUSA_ASSET_CDN_URL_MAP` is configured. Missing invalidation credentials should leave rows retryable without restoring deleted figure rows or blocking the user-facing delete action.
...
- `/api/status/library-fun` returns authenticated visible-library corpus totals for the final Status-page fun stats section. It excludes staged, queued, paused, failed, cleared, restored-paused, and soft-deleted operational rows, counts parsed words from preferred page text, counts indexed words from `Document.search_text`, and reports coverage totals for DOI values, verified citations, bibliography references, project resources, notes, annotations, domains, and tags.
- `/api/cache/status` returns authenticated Valkey/cache health, memory, policy, key count, hit/miss/eviction/expiry counters, per-family backend cache counters, last refresh/hydration timestamps, hot-route p50/p90/p95/p99 request metrics, active queue depth and oldest active-work age, largest PostgreSQL relation footprints, and key Medusa storage footprints. The profile menu and Status cache card show used-memory utilization against the active Valkey limit, with the profile menu adding a mini utilization graph; the Status Cache Details panel includes a Valkey resource monitor for used memory, limit, peak, RSS, keys, evictions, expiries, connected clients, and ops/sec. Active cache hydration status includes real planned/completed payload counts, planned document counts, hydrated-key writes, already-hot cache hits, bypasses, and errors; the shell and Status surface poll this status at the active-work cadence while hydration is running. `/api/cache/refresh` manually advances all durable cache revision families, records the refresh timestamp, warms dashboard, Library Fun Stats, organization lists, and the first Library page, and leaves old keys to expire or be evicted because revision-aware keys make them unreachable. `/api/cache/quench` flushes resident entries from the selected Valkey database without advancing revisions or touching PostgreSQL-backed source data; it is blocked while cache hydration is actively warming so the flush cannot race new writes. `/api/cache/hydrate` does not advance revisions; it preloads the current revision's safe derived payloads from PostgreSQL, walking all deterministic Library filter/sort pages, active saved-search result pages, visible document-detail plus adjacent document payloads, and major project/Recon/Portfolio/finance/job/backup/review/note surfaces subject to the same payload cap and Valkey LRU policy as read-through caching. `/api/cache/hydrate/pause`, `/api/cache/hydrate/resume`, and `/api/cache/hydrate/stop` control the in-process hydration loop cooperatively so rapid-development restarts can skip broad warming without changing cache revisions or source data. Valkey outages degrade to cache misses or unavailable status instead of making PostgreSQL-backed app reads fail.
...
- `/api/release/status` returns authenticated release status from the host-agent status file plus current runtime identity. `/api/release/history` remains an authenticated host-agent audit endpoint backed by `data/deploy/release-history.json`, but release history is no longer surfaced as a primary cockpit page. `/api/release/upgrade` writes an authenticated upgrade request file when a newer upstream release is available and the host agent marked apply as available. If the server is already on a newer build than the browser bundle, the same status response tells the frontend to reload instead of requesting another deploy; the initiating upgrade flow keeps the UI blocked until release completion, `/api/health`, the app shell route, and the cache-busting browser reload have all completed enough for the new bundle to take control.
- `/api/slipstream/status`, `/api/slipstream/enrollments`, `/api/slipstream/clients/{client_id}/disable`, `/api/slipstream/clients/{client_id}/revoke`, `/api/slipstream/leases/{lease_id}/cancel`, `/api/cloud-run/workers/status`, and `/api/cloud-run/workers/scale-plan` are authenticated admin routes used by Settings. Remote clients use `/api/slipstream/register` with a one-time enrollment token, then signed requests to `/api/slipstream/check-in`, `/api/slipstream/leases/claim`, `/api/slipstream/leases/{lease_id}/heartbeat`, `/api/slipstream/leases/{lease_id}/artifact`, `/api/slipstream/leases/{lease_id}/events`, `/api/slipstream/leases/{lease_id}/results`, and `/api/slipstream/leases/{lease_id}/fail`.
...
- Restored queued/running/paused import and Concordance jobs are parked as `restored_paused` by default so a worker cannot accidentally replay old backup state. `--reactivate-jobs` exists for deliberate maintenance use.
...
- DOI/source-link resolution is implemented for import enrichment and `citation_refresh` across local metadata/text/Bibliography evidence, Crossref title and bibliographic query matching, Semantic Scholar, OpenAlex, DataCite, Europe PMC, OpenCitations Meta, DOI.org Registration Agency checks, targeted title web evidence, and stable source-link fallback. Field-level evidence comparison and richer review UI for conflicts remain future work.
...
- Slipstream has the secure enrollment/signature/lease/result-apply foundation plus a basic PDF text-import client and a default-disabled Cloud Run worker-pool profile. Richer capability-specific remote runners, central scoped provider-call proxying for model steps, asset-bundle uploads beyond manifest-driven document/page/capability updates, automatic Cloud Run scaling execution, idle operational cost attribution, and broader end-to-end UI smoke coverage remain future work.
...
- Add robust citation verification beyond Crossref basics and richer field-level evidence review.
...
### 2026-07-02: CDN-gated durable asset delivery

Decision: Let Medusa offload immutable GCS-backed research assets through a dedicated Cloud CDN asset host while keeping app-owned authorization and private bucket posture.

Why: Original PDFs, extracted figure assets, and stored page-preview images are large and change rarely. Streaming them through the FastAPI application ties browser asset fan-out to backend worker/API capacity, which becomes painful when many figures or previews load at once.

Consequences:

- The GCS-backed asset host uses a global external Application Load Balancer backend bucket with Cloud CDN, one IPv4 address, and one IPv6 address. The shorthand "NLB" should not be used for this topology because GCS backend buckets are HTTP(S) Application Load Balancer resources.
- `MEDUSA_ASSET_CDN_BASE_URL` enables CDN redirects for `/api/documents/{document_id}/original`, `/api/figures/{figure_id}/asset`, and stored page-image routes. Without that setting, the existing authenticated streaming/rendering behavior remains the local/no-CDN fallback.
- Cloud CDN signed URL settings preserve private research assets without making the bucket public. The CDN backend bucket owns a signed request key, and the project's Cloud CDN cache-fill service account has bucket object-viewer access for origin fetches. Authenticated API routes remain the authorization gate and issue short-lived redirects; expiring signed URLs must not be embedded in cacheable document-detail JSON.
- Future URLs for immutable originals, figure images, and persisted page snapshots should reflect the asset-host scheme: clients may start at authenticated `/api/...` routes, but the bytes should come from short-lived signed `https://assets.medusa.evan.engineer/...` URLs whenever GCS/CDN is configured.
- The current page-preview image route is CDN-backed only for pages with a persisted `DocumentPage.image_uri`. If no durable page image exists, the route renders a PNG from the original PDF through the app as a deliberate fallback until the page-preview asset backlog item is implemented.
- Deleted figure assets move through `asset_deletion_queue`. The worker performs best-effort hourly object deletion and URL-map invalidation after rechecking that no current row still references the same URI.
...
- Replacement creates a normal one-document `ImportBatch` and queued `ImportJob`; it is not a parallel importer. The queued/running/paused/failed/ready visibility contract remains the same as ordinary imports.
...
- Cached v1 API families include `documents:list`, `documents:summary`, `documents:detail`, document-adjacent detail surfaces, `dashboard`, `status:library_fun`, organization/publication/preferences chrome, finance summaries, Recon, Portfolio, backup, review, notes, and selected job/Concordance payloads; Valkey failures become misses.
...
- Backend startup and manual Hydrate Cache preload the current revision from PostgreSQL without bumping revisions, report active hydration status/progress through `/api/cache/status`, and keep concurrent committed writes safe because any older hydrated payloads become unreachable through revision-aware keys.
- Manual Quench Cache flushes resident Valkey entries without bumping revisions, providing a debugging reset for cache inventory/content issues while preserving PostgreSQL as the source of truth.
- Status and the profile menu expose cache utilization, used memory, hit-rate matching, mode, per-family counters, refresh/hydration controls and progress, a Valkey resource monitor, and last refresh/hydration timestamps; Settings exposes the saved Valkey Memory Limit and applies it to the running service on Save All; `/api/*` cached responses expose `X-Medusa-Cache` and `X-Medusa-Cache-Family`.
...
- This was superseded on 2026-07-01 by the exhaustive DOI/source-link resolver. The remaining follow-up is field-level evidence comparison and richer review UI, not additional provider plumbing.
...
- The release service reads `data/deploy/release-status.json` and can expose `data/deploy/release-history.json` for authenticated audit use, while the app writes `data/deploy/release-request.json`; all live under ignored runtime data.
...
### 2026-07-01: Exhaustive DOI/source-link citation resolution

Decision: Implement exhaustive DOI/source-link resolution for import enrichment and `citation_refresh`. APA citations prefer DOI links when a DOI can be located; if no DOI is available, they use the best direct stable source link, preferably a PDF or other static document.
...
- Citation refresh is more exhaustive than the old DOI regex plus Crossref DOI/title/author/year matching path.
- DOI discovery inspects document metadata, extracted text, Bibliography/reference text, Crossref bibliographic query matching, Semantic Scholar, OpenAlex, DataCite, OpenCitations Meta, Europe PMC/PubMed evidence, DOI.org Registration Agency checks, and targeted title web evidence.
- Every attempted source, compact candidate list, conflict, confidence score, Registration Agency result, and fallback source-link choice is recorded as document metadata evidence and copied into Queue citation-candidate metadata when review is needed.
...
- `/api/imports/jobs/process-staged` promotes all staged import jobs to `queued`, records a processing event, and sets document processing status so the worker can claim them through the existing import pipeline. `/api/imports/jobs/pause` and `/api/imports/jobs/resume` cooperatively move queued import work between `queued` and `paused` without touching ready Library documents; `/api/imports/jobs/stop` clears queued/paused import rows through the existing non-Library queue cleanup path and reports running jobs as skipped rather than racing active worker threads.
...
- Metadata exports include Inquests as document children; restored queued/running/paused rows are parked unless restore is explicitly allowed to reactivate jobs.
...
- Queued Concordance jobs can be paused and resumed from the header control surface. Stopping a queued or paused Concordance run marks those jobs failed with user-stop evidence and refreshes the run totals; running jobs are reported as skipped so the backend does not race in-flight worker work.
...
- `/api/dashboard` includes import queued/running/paused counts plus active batch progress totals, active step, elapsed seconds, and a bounded, freshness-windowed list of failed AI-provider usage rows so the shell can render active progress and live failed-call alerts without replaying older Finances ledger failures.
- The active-work progress control is visually hidden when no imports or background runs are queued/running/paused but keeps its reserved header slot.
...
- The app shell records a local "starting" job immediately, reconciles it with persisted Concordance run/job state, and displays starting/queued/running/paused status in the header active-work control. After a short grace window, the dashboard's active Concordance count is authoritative so optimistic local background chips do not remain stuck when the server has no active Concordance work.
...
- The header active-work progress slot is wider and includes current known import spend while imports are queued/running/paused.
...
### 2026-07-02: Old-scan bibliography marker recovery

Decision: Let local PDF-span bibliography extraction recover mid-page References sections in old scanned articles when the heading begins inside a later column and numeric markers are OCR-damaged.

Why: Some older journal PDFs have a valid visible reference list but expose it as a noisy text layer: `References` may appear after body text in the right column, markers such as `[1]`, `[9]`, or `[10]` may be extracted as `[‘I`, `[Y]`, or `[IO]`, and lower-page author biographies can be interleaved after the reference band. Treating that as `not_found` blocks both import-time bibliography extraction and document-scoped Bibliography Refresh.

Consequences:

- `bibliography_extraction` capability version 5 recognizes OCR-damaged structural reference markers and embedded reference headings without accepting publisher reference-count boilerplate.
- PDF line ordering keeps the top reference band ahead of lower author-bio bands so page-end biographies do not hide the rest of the reference list.
- Deterministic extraction may preserve imperfect OCR text from the original PDF; forced Bibliography Refresh can still pass the recovered list through the selected Bibliography Cleanup model for APA-style repair.
...
- The active lock icon uses the same blue verified-field treatment as manually verified document fields. Verified DOI/APA/Bibliography/Summary badges and Library verified-state omission rules remain unchanged.

### 2026-06-30: Publication metadata and browse-by-publication

Decision: Model publications, books, textbooks, proceedings, report series, and other containers as first-class entities through `Publication`, `PublicationAlias`, and `DocumentPublication`, while keeping `Document.journal` and `Document.publisher` as compatibility mirrors of the primary publication title and publisher.

Why: A document's publication relationship is more than citation decoration. Medusa needs to preserve the shared container identity so Library browsing can answer "what else do I have from this journal/book/proceedings?" while still storing per-document appearance details such as volume, issue, pages, article number, edition, chapter, series, and event metadata for citations.

Consequences:

- Import metadata extraction returns a structured publication candidate using the existing Metadata model path, defaulting to `gpt-5.5`; `gpt-5.4` remains a manual cost-saving override through task preferences, but publication detection stays on the citation-critical metadata default.
- Lookup and reconciliation prefer exact DOI/Crossref evidence, then OpenAlex/Semantic Scholar, then ISBN/ISSN/title evidence from bounded no-credential book providers, and only then GPT-only extraction or legacy `Document.journal`/`publisher` fallback.
- Existing documents are not backfilled by migration or default broad selections. They receive publication rows only through explicit document refresh or an opt-in `publication_metadata` Concordance scope.
- `publication_metadata` is a versioned Concordance capability for targeted document/scope runs, but it is excluded from default broad UI selections because old documents should not be surprise-backfilled.
- Publication data participates in document detail/list payloads, metadata export/restore, search rebuilds, citation metadata assembly, cache invalidation, and `DocumentVersion` snapshots.
- `/api/publications` exposes searchable publication rows with ready-document counts, and `/api/documents/list?publication_id=...` is the Library browse path used by active filter chips and the detail pane's Other from publication action.
