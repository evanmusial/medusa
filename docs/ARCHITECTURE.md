...
- `backend/app/slipstream/client.py`: bundled outbound Slipstream runner for import preprocessing, with signed polling, concurrent lease workers, heartbeat threads, local state/key storage, raw-text extraction, partial result manifests, and best-effort CPU affinity/QoS hints for local laptop operation.
