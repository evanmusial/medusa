...
- Optimize can now suggest downgrading, retiring, merging, or pruning low-use tags even when the selected scope has no model-proposed merge candidates. True zero-link orphan tags are deterministic cleanup candidates: merge them into a useful used tag when variant, broader-prefix, or high-similarity checks find a target; otherwise prune the unused tag row entirely. One-use candidates are retire candidates when they have no durable score evidence or weak evidence; one-use canonicals are downgrade candidates until repeated use proves durable value; singleton assignments without scoring history can be pruned by explicit approval. These remain user-approved operations, not automatic cleanup.
