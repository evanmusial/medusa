...
- Horizontal work navigation replaces the old left sidebar: Library, Domains, Projects, Tags, Queue, Notes, Import, Stashes, Budget & Costs, and Settings are laid out left-to-right in the former dashboard-metric strip, styled as quiet no-background links with Settings pushed to the far right. Each top-level workspace has a canonical browser path (`/library`, `/domains`, `/projects`, `/tags`, `/queue`, `/notes`, `/import`, `/stashes`, `/budget`, and `/settings`) so pages can be bookmarked, opened in a new tab, and restored on refresh. Document focus links use `/documents/{document_id}` and open Library with that document selected in the detail pane; `/document/{document_id}` is accepted as a compatibility alias. Top navigation assigns single-letter shortcuts when focus is not inside an editable control: `L` Library, `D` Domains, `P` Projects, `T` Tags, `Q` Queue, `N` Notes, `I` Import, `A` Stashes, `B` Budget & Costs, and `S` Settings; top-nav tooltips show the shortcut on a second line.
...
- The quiet horizontal navigation renders as links to `/library`, `/domains`, `/projects`, `/tags`, `/queue`, `/notes`, `/import`, `/stashes`, `/budget`, and `/settings`.
