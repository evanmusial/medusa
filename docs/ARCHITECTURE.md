...
- Motion for React is the standard runtime animation layer for restrained cockpit motion. Focused vertical reading/work panes such as Library rows, document detail, Reader PDF/text, and compare-mode panes show thin accent-colored scroll progress rails at the top of the scroll area. Single-purpose buttons may use small hover/tap gestures that do not change layout, while async buttons keep the existing progress/feedback language. First-load or buffered content uses fixed-footprint shimmer skeletons where the content shape is known. Medusa-owned dialogs, command surfaces, tabs, and chips may use short opacity/scale or selected-marker transitions, all honoring `prefers-reduced-motion`.
