...
- Settings > Preferences includes a browser-local Button Motion palette for comparing shared async button and dialog feedback treatments. The default `Rail Only` style keeps non-circular busy glyphs still while preserving rotation for refresh/reload-style icons, and alternate palette choices move attention to the control rim, surface pulse, or static state marker without changing backend job semantics.
