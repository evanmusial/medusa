...
- The header build stamp sits in the action cluster before the theme toggle and uses the frontend build timestamp (`YYYY.MM.DD-HHMM`) so the running UI can be identified without opening developer tools. `MEDUSA_BUILD_DATE` can override the stamp for release or CI builds.
