...
- Tags view supports tag management as a sortable, searchable table with document counts, row selection, and a left-aligned operation toolbar for Clear Selection, Rename, Merge, Delete, and Optimize. Rename works for exactly one selected tag. Merge is enabled for two or more selected tags, displays the selected count in the button label, and opens a confirmation dialog where the user chooses a selected tag to keep or enters a different merged tag name. Optimize uses `gpt-5.4-mini` to review the selected tags, or the currently visible filtered tag list when nothing is selected, and returns reviewable merge suggestions with rationale, confidence, model label, source tag counts, and server-computed affected-document totals. No optimization is applied until the user approves a suggestion; approved suggestions call the normal audited merge endpoint so every affected document keeps or receives the target/new tag before source tags are removed. Delete remains a reserved control pending implementation.
...
- Full database backup and restore are browser-driven from Settings, including a recent backup/restore history list so prior runs remain visible after a new backup completes. Legacy metadata restore remains CLI-first: dry-run by default, explicit `--apply`, and intended for JSON export drills or partial fresh-database recovery.
...
- `Tag`: flat keyword/topic label. Tag API responses include an active-document count for management views.
...
- `summary_refresh` v1: button-scoped capability used by document Summary Check. It regenerates only the main Markdown `rich_summary` using the selected Summary model, records usage with task key `summary`, rebuilds document search when the summary changes, and keeps broader metadata/topic extraction out of one-off summary checks. It is accepted by the Concordance API for explicit Summary Check requests but is not included in default all-capability Concordance selections.
...
- Backups use `pg_dump --format=custom`, zstd compression, object names shaped like `medusa-postgres-YYYYMMDD-HHMMSS-<short-hostname>.dump.zst`, and sibling JSON manifests under `<GCS_PREFIX>/backups/`. Settings shows a likely backup size under the Backup Database button plus the ten most recent backup/restore runs; the estimate uses `pg_database_size(current_database())` and, after a completed backup exists, scales by the latest recorded compressed-size/source-database-size ratio.
