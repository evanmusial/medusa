...
- Fixed top header with the Medusa emblem, global search, a reserved active-work progress control, optional release upgrade action, icon-only Status action, theme toggle, and a far-right user options menu. The user menu exposes Settings and Sign out, replacing the former dedicated logout icon. The active-work slot keeps its width while idle so the header does not jump when imports, Concordance, citation refresh work, or release prompts start, but it must shrink and ellipsize before it can collide with the global search field under browser zoom. The release action appears as a compact accent-colored `Upgrade Now` button to the left of the day/night theme switch when a host-side status check reports newer pushed code, and switches to `Reload Now` when the server is already running a newer build than the current browser bundle. The icon-only Status action opens `/status`, showing build/version identity with the short commit hash, uptime, memory and disk footprints, database size, runtime versions, proxy state, storage path details, and backend image identity from existing authenticated status APIs. Its final section is Library Fun Stats: visible-library document/page/figure counts, bibliography reference totals, parsed and indexed word/character counts, author/DOI/citation coverage, project-source use, notes, annotations, domains, and tags. Docker image/layer sizing remains optional when the Docker Engine socket is deliberately mounted; without that socket, the Status and Utilities backend-image cards fall back to the backend base-image identity reported by runtime inventory and state that layer sizing is disabled. Starting an upgrade warns that unsaved edits will be discarded, locks the initiating browser tab behind a release progress overlay, waits for the host agent to report completion and for both `/api/health` and the app shell route to answer through the public proxy, then reloads into the new browser bundle before returning control. If a browser is already stale when the user clicks `Reload Now`, the same health and app-shell checks run before the cache-busting reload begins.
