...
26. Stored `rich_summary` text must begin with the semantic substance of the summary itself, not a standalone heading such as `Summary`, `Overview`, `Abstract`, `Synopsis`, or similar, and not with a single-word opener. The AI prompts also request complete-sentence default summaries as technical plain-text paragraphs written at a graduate academic level suitable for a master's-degree reader: the first paragraph states the paper's broad facts and purpose, key findings and concrete facts appear early, sentences avoid starting with prepositions, and subsequent paragraphs summarize the main points, ideas, methods, findings, and concepts without bold, italics, bullets, em dashes, or fancy quotes unless the user explicitly asks for another format. Import/Concordance cleanup strips standalone first-line headings before persistence.
