...
- Domains view is the domain-tree management surface. It provides a searchable alphabetized tree, top-level or nested domain creation, selected-domain editing for name, parent, description, and color, child creation, soft delete with confirmation, and a direct list of documents assigned to the selected domain. Top-level domains and every nested sibling group are displayed alphabetically by name.
