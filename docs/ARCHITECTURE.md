...
- Settings includes a quiet Slipstream panel for enablement status, one-time enrollment tokens, registered clients, capability/capacity metadata, last check-in, active leases, and disable/revoke/cancel controls. A Cloud Run panel exposes the disabled-by-default worker-pool profile with enablement, numeric target concurrency, a CPU/memory flavor dropdown, active Cloud Run clients/leases, generated `gcloud run worker-pools` commands, and typical runtime cost estimates. Queue rows show whether a running job is assigned to a local worker, `Slipstream: client-name`, or Cloud Run through the same lease heartbeat/expiry metadata.
