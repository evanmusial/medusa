...
- `/api/dashboard` includes import queued/running counts plus active batch progress totals, active step, elapsed seconds, and a bounded list of recent failed AI-provider usage rows so the shell can render active progress and failed-call alerts without scanning the full job or Finances ledgers.
