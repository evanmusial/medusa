...
- Horizontal work navigation replaces the old left sidebar: Library, Domains, Projects, Tags, Queue, Notes, Import, Stashes, Budget & Costs, Utilities, and Settings are laid out left-to-right in the former dashboard-metric strip, styled as quiet no-background links with Settings pushed to the far right. Each top-level workspace has a canonical browser path (`/library`, `/domains`, `/projects`, `/tags`, `/queue`, `/notes`, `/import`, `/stashes`, `/budget`, `/utilities`, and `/settings`) so pages can be bookmarked, opened in a new tab, and restored on refresh. Document focus links use `/documents/{document_id}` and open Library with that document selected in the detail pane; `/document/{document_id}` is accepted as a compatibility alias. Top navigation assigns single-letter shortcuts when focus is not inside an editable control: `L` Library, `D` Domains, `P` Projects, `T` Tags, `Q` Queue, `N` Notes, `I` Import, `A` Stashes, `B` Budget & Costs, `U` Utilities, and `S` Settings; top-nav tooltips show the shortcut on a second line.
- Main Library view uses a tri-pane layout:
  - Resizable left filter pane for domains, tags, smart filters, saved searches, and a Title Cleanup utility under the Domains tree. The pane has a content-aware minimum so select controls and their affordances remain visible. Title Cleanup trims leading/trailing title whitespace and collapses repeated title whitespace across active documents, using the same async button feedback treatment as Refresh and Concord actions.
...
- The header build stamp sits in the action cluster before the theme toggle and uses `YYYYMMDD (BUILD_NUMBER)` so the running UI can be identified without opening developer tools. `MEDUSA_BUILD_DATE` and `MEDUSA_BUILD_NUMBER` can override the stamp for release or CI builds; local builds use the current date and build time when those values are blank.
...
- `ModelPricingRecord`: model-pricing history for OpenAI and Google models with provider, model, standard per-million token rates, optional Gemini over-200k rates, source URL, observed/check timestamps, and `superseded_at` for price changes. Refreshes update `last_checked_at` on unchanged active prices and create a new historical row only when the rate signature changes.
...
- Second-pass state: import-processing presets currently live in `AppPreference`, import batches/jobs/documents snapshot the selected preset in evidence, `Document.bibliography` stores extracted source reference lists, and `Figure` rows store local visual crops with geometry, labels, captions, source kind, orientation metadata, and local context. Remaining planned entities include layout blocks, first-class structured table rows/cells, visual asset candidates, visual audit warnings, and richer figure context/crop-quality records so raw extraction, cleaned body text, removed boilerplate, source reference lists, and derived visual assets can be inspected or regenerated independently.
...
- AI usage accounting is stored separately from document metadata in `OpenAIUsageRecord` so cost/debug history survives metadata correction and can include failed calls. The ledger records usage reported by OpenAI and Gemini calls. Budget & Costs estimates dollars from `ModelPricingRecord` history when available, falling back to the current known standard-pricing table for enabled OpenAI GPT model families, OpenAI embedding models, and Gemini Developer API models. Usage cost lookup resolves model aliases and timestamp-matches each call to the price row active when the call happened; unknown or unavailable models remain unpriced, and token counts stay the durable source of truth because model pricing and project access can change outside the app. `DocumentCompositionRecord` is the document-facing provenance layer over that ledger: it preserves the staged cost estimate, actual import pipeline, and stage/model composition later Concordance logic can inspect before deciding whether a document already satisfies a capability/model requirement.
...
20. Extraction and async document-intelligence work are split into Settings-selectable tasks: Raw Text Extraction, Metadata, Summary, APA Citation Matching, Tag Suggestions, Text on Pages (Normalization), Text Chunk Encoding, and Accessory Summaries. The compatible internal key for Tag Suggestions remains `keywords_topics`, but persisted output is flattened into tags. Metadata and APA fallback matching default to `OPENAI_MODEL=gpt-5.5`; Summary defaults to `gpt-5.4`; Tag Suggestions defaults to `gpt-5.4-mini`; Accessory Summaries defaults to `gpt-5.4`; Text Chunk Encoding defaults to `OPENAI_EMBEDDING_MODEL` and offers `text-embedding-3-small`, `text-embedding-3-large`, and `text-embedding-ada-002`.
...
25. During imports, `DocumentCompositionRecord` rows are written for the staged cost estimate, each local stage, and synced per-call AI usage rows. Rows record stage order, method/model/provider, known dollar cost, local duration, tokens, status, processing warnings/errors, and pipeline metadata. Estimate rows are not counted as actual provider spend; Composition displays them separately as estimate-vs-actual comparison. Older documents without rows intentionally show Composition as not available.
26. Stored `rich_summary` text must begin with the semantic substance of the summary itself, not a standalone heading such as `Summary`, `Overview`, `Abstract`, `Synopsis`, or similar, and not with a single-word opener. The AI prompts also request complete-sentence default summaries as technical plain-text paragraphs: the first paragraph states the paper's broad facts and purpose, and subsequent paragraphs summarize the main points, ideas, methods, findings, and concepts without bold, italics, bullets, em dashes, or fancy quotes unless the user explicitly asks for another format. Import/Concordance cleanup strips standalone first-line headings before persistence.
27. DOI is discovered from AI metadata and local DOI regex over extracted text. Crossref lookup is attempted by DOI first, otherwise by title with optional first-author and publication-year constraints. If Crossref evidence is available, missing citation fields are filled from that evidence without overwriting existing values.
...
7. `visual_asset_extraction` now runs local extraction for embedded raster images, displayed page-image regions, and vector drawing clusters, recording durable crops with page geometry, source kind, orientation, captions, and basic warnings. Exhaustive full-page scan/table-region coverage, duplicate/incomplete crop audit, and complete axes/legend expansion remain planned.
...
10. Composition records second-pass stages, warnings, model/provider choices, local duration, token/file-context usage, and costs where rows and usage data exist. A more explicit preset-snapshot display and richer second-pass warning surfaces remain planned.
...
- The document detail pane estimates a current-document Concordance Run before asking the user to confirm the run.
...
- second-pass document cleanup, visual asset extraction, visual asset context, structured tables, and OCR fallback
- tag extraction
...
- `/api/openai/usage` returns authenticated usage totals, task/model rollups, recent OpenAI/Gemini call records, pricing status, and conservative estimated costs for a requested period (`last_day`, `last_month`, `last_3_months`, or `all_time`). Unknown models are counted as unpriced rather than guessed.
- `/api/model-pricing/refresh` records the current enabled OpenAI and Google standard-pricing snapshot into `ModelPricingRecord`, updating unchanged rows' `last_checked_at` and appending a new row only when a model price changes. Settings warns when the latest active pricing check is more than two days old.
- `/api/utilities/database/status` reports database-maintenance counts for Utilities, including the hidden import cache count used by the Clear Import Cache button. `/api/utilities/database/compact` runs PostgreSQL `VACUUM (FULL, ANALYZE)` through an autocommit connection. `/api/utilities/database/optimize` runs PostgreSQL `ANALYZE`. `/api/utilities/import-cache/clear` removes hidden terminal import document rows, their terminal import jobs, stale `ProjectItem` links, managed processing-cache files, and staged original objects where deletion is available. It must not remove Library-visible documents or staged/queued/running/failed/restored-paused import rows. `/api/utilities/container/status` includes a runtime-version inventory for HAProxy, backend Python/package/system binaries, frontend Node/Vite, the frontend build stamp, and optional current-backend Docker image/layer sizing when `MEDUSA_DOCKER_SOCKET_PATH` points at a mounted Docker Engine socket. `/api/utilities/haproxy/status` returns authenticated HAProxy stats derived from the internal CSV feed and degrades to an unavailable status when the backend is not running inside the Compose deployment.
...
- New second-pass capabilities must be implemented both at import time and retroactively through Concordance Runs while protecting user-edited text from silent overwrite.
...
- Settings exposes eight extraction/analysis model controls inside Import Processing as shared defaults: Raw Text Extraction, Metadata, Summary, APA Citation Matching, Tag Suggestions, Text on Pages (Normalization), Text Chunk Encoding, and Accessory Summaries.
...
- The shared summary prompt hint applies to document summaries and Accessory Summaries. Defaults are technical plain-text paragraphs with complete sentences, no standalone Summary/Overview-style opening, no single-word opener, no bold, italics, bullets, em dashes, fancy quotes, or decorative Markdown unless the user explicitly asks for another format.
- Import and Concordance summary persistence strips standalone first-line summary headings before storing `rich_summary`.
...
Decision: Store model pricing in `ModelPricingRecord` history and expose pricing freshness plus a Refresh Models & Pricing action alongside the model defaults in Settings > Import Processing.
...
- Settings shows the latest local pricing refresh timestamp, the source snapshot date, a Refresh Models & Pricing button using the standard async button animation, and a warning when pricing has not been refreshed in more than two days.
- Refreshing stores current enabled OpenAI GPT/embedding and Google Gemini standard paid-tier token prices. It updates `last_checked_at` for unchanged prices and inserts a new active row only when the rate signature changes, setting `superseded_at` on the prior row.
- Budget & Costs, active import spend, import cost exemplars, and Composition sync cost each usage record against the historical price row active at the usage timestamp, with the static current pricing table as fallback when no database history exists yet.
...
- Stashes view lists saved DOIs with local sorting, per-row DOI copy, manual Sci-Hub DOI links in a new window, resolver-backed Import DOI, Upload PDF, and compact dashed drag target controls.
