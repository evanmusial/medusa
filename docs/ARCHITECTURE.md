...
- Settings and Utilities share one configuration-workbench language: full-width open sections separated by quiet horizontal rules, normal-weight 14px explanatory text, section headings with restrained icon treatment, and fact readouts arranged as row/column separators instead of individually boxed stat cards. Bounded controls such as dropzones, charts, dialogs, and dense editable preset groups may still have a subtle frame when the boundary supports interaction, but ordinary settings, maintenance facts, usage numbers, runtime facts, and backup status should not look like separate dashboard tiles.
