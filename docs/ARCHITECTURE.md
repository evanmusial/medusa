...
- Import view also provides active drop-target feedback, duplicate-decision handling, and live job rows that shade left-to-right with per-file progress while showing bold status, current model, and spend so far.
