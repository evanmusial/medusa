...
- The reserved header active-work control shows import progress first when imports are active, including imported/total count, percentage, ETA in minutes, current stage, and current known dollar spend so far, otherwise active Concordance/citation background work. When an import batch completes, the header shows the completed batch's actual cost and keeps that notice visible until the browser user dismisses it. It is visually hidden when no work is active or retained completion notice exists, keeps its wider layout slot reserved, and clicking it opens Queue.
