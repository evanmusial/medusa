...
- `backend/app/services/slipstream.py`: remote-processing enrollment, request-signature verification, enrollment capability/capacity limits, lease coordination shared by local and remote workers, artifact access, heartbeat/expiry, import-preprocess result application, and result-application helpers.
- `backend/app/slipstream/client.py`: bundled outbound Slipstream runner for import preprocessing, with signed polling, concurrent lease workers, heartbeat threads, local state/key storage, raw-text extraction, partial result manifests, one-at-a-time claim ramping, empty/error claim backoff, transient check-in retry behavior, and best-effort CPU affinity/QoS hints for local laptop operation.
