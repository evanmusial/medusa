...
Planned second-pass extension on `codex/second-pass-document-processing`:
...
4. `ocr_fallback` runs only for low-text or scanned pages when the selected preset and available credentials permit it. Google Vision is the intended first cloud OCR provider, with local/no-credential fallback behavior preserved.
...
Decision: Store model pricing in `ModelPricingRecord` history and expose pricing freshness plus a Refresh Models & Pricing action at the end of Settings > Models.
...
- Settings > Models groups compatible text-generation choices under OpenAI and Google, and excludes Gemini model ids containing `preview` plus deprecated/shutdown Gemini defaults from the Google section. Current built-in Google options include Gemini 3.1 Flash-Lite, Gemini 2.5 Pro, Gemini 2.5 Flash, Gemini 2.5 Flash-Lite, and stable latest aliases that resolve to those priced families.
