...
- The header build stamp sits in the action cluster before the theme toggle and uses `YYYYMMDD (HASH)` so the running UI can be identified without opening developer tools. `MEDUSA_BUILD_DATE` and `MEDUSA_BUILD_HASH` can override the stamp for release or CI builds; local builds use the current date and a build-content hash when those values are blank.
