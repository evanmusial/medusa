...
- The top header does not display a build/version stamp; runtime identification stays in Utilities. The frontend build stamp still uses `YYYYMMDD (HASH)` in runtime metadata so the running UI can be identified without developer tools. `MEDUSA_BUILD_DATE` and `MEDUSA_BUILD_HASH` can override the stamp for release or CI builds; local builds use the current date and a build-content hash when those values are blank.
